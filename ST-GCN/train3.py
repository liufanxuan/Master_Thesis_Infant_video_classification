...
def train(args, net, train_set, val_set, test_set, save_dir, an_id):
    weights = [1.0,7.0]
    ...
    with mf.start_run():
        ...
        test_roc_auces, test_acces, val_roc_auces, val_acces, train_roc_auces, train_acces, epoches, val_TFs, val_FFs, test_TFs, test_FFs = [],[],[],[],[],[],[],[],[],[],[]
        for epoch in range(1, args.max_epoch + 1):
            ...
            if 1:
                ...
                #test_roc_auc = evaluate_CM(net=net, dataset=test_set, suffix_len=args.suffix_len, device=device)
                test_roc_auc, test_acc, test_loss,TF_test,FF_test = evaluate_CM_validation(net=net, dataset=test_set, suffix_len=args.suffix_len, device=device)
                train_roc_auc, train_acc, train_loss = evaluate_CM(net=net, dataset=train_set, suffix_len=args.suffix_len, device="cuda")
                val_TFs.append(TF)
                val_FFs.append(FF)
                test_TFs.append(TF_test)
                test_FFs.append(FF_test)
                print(f"Epoch: {epoch:03}. [train] confusion_matrix: ")
                print(f"{train_roc_auc} ")
                print(f"[val] confusion_matrix: ")
                print(f"{val_roc_auc} ")
                ...
                test_roc_auces.append(test_roc_auc)
                ...
                print(f"[test] roc_auc: {test_roc_auc:.4f}, acc: {test_acc:.4f}, loss: {test_loss:.4f}")
        ...
        epoch_df = epoch_df.append(pd.DataFrame({'epoch': epoches,
                                                 ...
                                                 'test_roc_auc': test_roc_auces,
                                                 'val_TF': val_TFs,
                                                 'val_FF': val_FFs,
                                                 'test_TF': test_TFs,
                                                 'test_FF': test_FFs}))
        epoch_df = epoch_df[['epoch', 'train_acc', 'val_acc', 'test_acc', 'train_roc_auc', 'val_roc_auc','test_roc_auc', 'val_TF','val_FF','test_TF','test_FF']]
...
def main(an_id):
    ...
    parser.add_argument('--max-epoch', type=int, default=50, help='Max epoch.')
    ...
    pose_data_root = Path(data_root, f'cooked_flipped_subsequences_0500_0100')
    fold_dir = Path(data_root, f'folds_flipped/split_absent_normal_same_without43')
    ...
    val_set = ClipDataset(data_root=pose_data_root, clip_size=args.clip_size, stride=args.stride,
                          ...
                          channels=args.channels)
    test_set = ClipDataset(data_root=pose_data_root, clip_size=args.clip_size, stride=args.stride,
                            length=args.length, label_file_path=test_label_file, suffix_len=args.suffix_len,
                            channels=args.channels)
   ...
   # th.cuda.manual_seed(0)
    ...
    save_dir = Path(fold_dir, f'500_7_lr_{args.lr}_weight_delay_{args.weight_decay}_clip{args.clip_size}_stride_{args.stride}_flipped')
    save_dir.mkdir(parents=True, exist_ok=True)
    train(args, net, train_set, val_set, test_set, save_dir, an_id)
    return 0
