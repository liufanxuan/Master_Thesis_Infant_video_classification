...
def load_data_split(data_root, train_label_dict, test_label_dict):
    ...
    return train_infant_list, train_labels, test_infant_list, test_labels
...
def stratified_split_random_new(target_dir, label_file_path, data_root, cv=5, random_state=42):
    ...
    fold_idx = 0
    for train_index, test_index in sss.split(infant_ids, infant_labels):
        id_train, id_test = infant_ids[train_index], infant_ids[test_index]
        label_train, label_test = infant_labels[train_index], infant_labels[test_index]   
        print(id_test)
        print(id_train)        
        ...
        train_infant_list, train_labels, test_infant_list, test_labels = load_data_split(data_root, train_label_dict, test_label_dict)
        train_file_path = Path(target_dir, f'train_{fold_idx:02}.txt')
        test_file_path = Path(target_dir, f'test_{fold_idx:02}.txt')
...
def main():
    ...
    data_root = Path(conf['data_root'], f'cooked_flipped') 
    label_file_path = Path(conf['data_root'], 'Label_absent_normal.csv')
    split_dir = Path(conf['data_root'], f'folds_flipped/split_absent_normal')
