...
def read_video(video_file_path):
    videogen = skvideo.io.vreader(video_file_path)
    new_videogen = itertools.islice(videogen, 0, 1, 1)
    for image in new_videogen:
        a = 1
    return image

def get_video_information(video_dir, verbose=False):
    video_file_paths = [file_path for file_path in Path(video_dir).rglob('*.mp4')]
    ...
    for i, file_path in enumerate(video_file_paths):
        ...
        fpsl.append(fps)
        img = read_video(file_path)
        nrows = len(img)
        ncols = len(img[0])
        rowlist.append(nrows)
        collist.append(ncols)
        video_file_names.append(file_name[-13:])
        if verbose:
            print(f"Done ({i + 1:003}/{len(video_file_paths):003})!! get_video_information: {file_name[-13:]}")
    df_fps['fps'] = pd.Series(fpsl)
    df_fps['pixel_x'] = pd.Series(collist)
    df_fps['pixel_y'] = pd.Series(rowlist)
    df_fps['video'] = pd.Series(video_file_names)
    return df_fps

def load_raw_pkl_files(pickle_dir, verbose=False, max_frame=None):
    pkl_file_paths = [file_path for file_path in Path(pickle_dir).rglob('*.pkl')]
    df_pkl = pd.DataFrame()
    for i, file_path in enumerate(pkl_file_paths):
        file_path = str(file_path)
        one_file = pd.read_pickle(file_path).reset_index().drop('index', axis=1)
        if max_frame is not None:
            one_file = one_file[one_file.frame <= max_frame]
        df_pkl = df_pkl.append(one_file)
        if verbose:
            print(f"Done ({i + 1:003}/{len(pkl_file_paths):003})!! data_utils::load_raw_pkl_files: {file_path}")
...
def edit_df(df, df_fps):
    ...
    # add keypoint labels
    bps = ["Nose", "Neck", "RShoulder", "RElbow", "RWrist", "LShoulder", "LElbow", "LWrist", "RHip", "RKnee",
...
def normalise_skeletons(df):
    ...
    bps = ["Nose", "Neck", "RShoulder", "RElbow", "RWrist", "LShoulder", "LElbow", "LWrist", "RHip", "RKnee", "RAnkle",
           "LHip", "LKnee", \
           "LAnkle", "REye", "LEye", "REar", "LEar"]
    ubps = ["Nose", "Neck", "RShoulder", "RElbow", "RWrist", "LShoulder", "LElbow", "LWrist", "REye", "LEye", "REar",
            "LEar"]
    ...
    df.loc[(df.ref_angle > np.pi) & (df.ref_angle < 2 * np.pi), 'ref_angle'] = 3 * np.pi - df.loc[
        (df.ref_angle > np.pi) & (df.ref_angle < 2 * np.pi), 'ref_angle']
    df['x_rotate'] = df['refx'] + np.cos(df['ref_angle']) * (df['x'] - df['refx']) - np.sin(df['ref_angle']) * (
                df['y'] - df['refy'])
    df['y_rotate'] = df['refy'] + np.sin(df['ref_angle']) * (df['x'] - df['refx']) + np.cos(df['ref_angle']) * (
                df['y'] - df['refy'])
...
def get_joint_angles(df):
    ...
    # compute angle here
    df_2['dot'] = (df_2['x1'] - df_2['x0']) * (df_2['x2'] - df_2['x0']) + (df_2['y1'] - df_2['y0']) * (
                df_2['y2'] - df_2['y0'])
    df_2['det'] = (df_2['x1'] - df_2['x0']) * (df_2['y2'] - df_2['y0']) - (df_2['y1'] - df_2['y0']) * (
                df_2['x2'] - df_2['x0'])
    ...
    df_info = df.groupby(['video', 'frame', 'fps', 'time', 'delta_t']).mean().reset_index()[
        ['video', 'frame', 'fps', 'time', 'delta_t']]
...
def get_dynamics_xy(xdf, delta_window):
    ...
    xdf = xdf.groupby(['bp', 'video']).apply(lambda x: get_delta(x, 'velocity_x', 'delta_velocity_x')).reset_index(
        drop=True)
    xdf = xdf.groupby(['bp', 'video']).apply(lambda x: get_delta(x, 'velocity_y', 'delta_velocity_y')).reset_index(
        drop=True)
    ...
    xdf = xdf.groupby(['bp', 'video']).apply(
        lambda x: smooth_dyn(x, 'acceleration_x_raw', 'acceleration_x', delta_window))
    xdf = xdf.groupby(['bp', 'video']).apply(
        lambda x: smooth_dyn(x, 'acceleration_y_raw', 'acceleration_y', delta_window))
...
def get_dynamics_angle(adf, delta_window):
    adf = adf.groupby(['bp', 'video']).apply(lambda x: get_angle_displacement(x, 'angle', 'displacement')).reset_index(
        drop=True)
    ...
    adf = adf.groupby(['bp', 'video']).apply(lambda x: get_delta(x, 'velocity', 'delta_velocity')).reset_index(
        drop=True)
